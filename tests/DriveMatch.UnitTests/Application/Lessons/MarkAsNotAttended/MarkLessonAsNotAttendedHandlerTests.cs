...
namespace DriveMatch.UnitTests.Application.Lessons.MarkAsNotAttended;

public class MarkLessonAsNotAttendedHandlerTests
{
    [Fact]
    public async Task HandleAsync_ShouldMarkLessonAsNotAttended_WhenLessonIsScheduled()
    {
        var lesson = CreateLesson();
        var unitOfWork = new FakeUnitOfWork();

        var handler = new MarkLessonAsNotAttendedHandler(
            new FakeLessonRepository(lesson),
            unitOfWork);

        var result = await handler.HandleAsync(
            new MarkLessonAsNotAttendedCommand(lesson.Id));

        Assert.Equal(lesson.Id, result.LessonId);
        Assert.Equal(LessonStatus.NotAttended, result.Status);
    ...
    public async Task HandleAsync_ShouldThrowLessonNotFoundException_WhenLessonDoesNotExist()
    {
        ...
        await Assert.ThrowsAsync<LessonNotFoundException>(
            () => handler.HandleAsync(
                new MarkLessonAsNotAttendedCommand(Guid.NewGuid())));
    }
